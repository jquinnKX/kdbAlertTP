/q aeRT1.q [host]:port[:usr:pwd] [host]:port[:usr:pwd]
/2008.09.09 .k ->.q
.proc.name:last[.z.x];
...
.ae.orderToTrade_wj_upd:{[t;x]
    if[(t=`dxOrderPublic);
        `x set select transactTime,sym,eventID,orderID,executionOptions,eventType,orderType from x where eventType=`Place;
        if[not count x;:()];
        startTime:.z.P;
        wBefore:.Q.w[];
        tsvector:system"ts:20 .ae.orderToTrade_wj_alert[x]";
        endTime:.z.P;
        wAfter:.Q.w[];
        .log.out -3!(`.ae.orderToTrade_wj_alert;startTime;endTime;min[x`transactTime];max[x`transactTime];tsvector[0];tsvector[1];wBefore`used;wAfter`used;wBefore`heap;wAfter`heap);
    ];
 };

/q aeID1.q [host]:port[:usr:pwd] [host]:port[:usr:pwd]
/2008.09.09 .k ->.q
.proc.name:last[.z.x];
...
.ae.orderToTrade_wj_alert:{[data]

    windows:(data[`transactTime]-0D00:05;data[`transactTime]);

    lookupTable:update rn:i from select sym,transactTime,limitPrice,originalQuantity,side from dxOrderPublic where 
...
.z.ts:{
    startTime:.z.P;
    wBefore:.Q.w[];
    tsvector:system"ts:20 outcome:.ae.orderToTrade_wj_ts[]";
    $[outcome~`noDataToAnalyse;:();.ae.orderToTrade_wj_lastEventAnalysed:outcome[0]];
    endTime:.z.P;
    wAfter:.Q.w[];
    .log.out -3!(`.ae.orderToTrade_wj_ts;startTime;endTime;outcome[1];outcome[2];tsvector[0];tsvector[1];wBefore`used;wAfter`used;wBefore`heap;wAfter`heap);
    if[count dxATAlert;
...
.ae.orderToTrade_wj_ts:{
    ...
    .ae.orderToTrade_wj_alert[dataToAnalyse];

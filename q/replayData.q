...
{[fileDate]
    ...
    if[fileDate~`readFromMemory;
        `dxOrderPublic_file set get hsym[`$raze[system"echo $HOME/kdbAlertTP/dxOrderPublic_file"]];
...
h:neg hopen`$"::",string[5000+.proc.portNumberIncrement];
...
.log.out"count before trim = ",string[count[dxOrderPublic_file]];

if[`replayFinishTime in key .proc.inputOptions;
    delete from `dxOrderPublic_file where not transactTime within (.global.timeToReplayFrom;value raze .proc.inputOptions[`replayFinishTime]);
 ];
 .Q.gc[];

.log.out"count after trim = ",string[count[dxOrderPublic_file]];
...
.global.wipeOfReplayedDataGap:0D00:20:00;
.global.nextWipeOfReplayedDataTime:.z.P+.global.wipeOfReplayedDataGap;
...
.z.ts:{
   ...
   if[.z.P>.global.nextWipeOfReplayedDataTime;
		.log.out"count before wipe = ",string[count[dxOrderPublic_file]];
		delete from `dxOrderPublic_file where i<.global.nextRowNumberToStartReplayingFrom;
		.log.out"count after wipe = ",string[count[dxOrderPublic_file]];
		.global.nextWipeOfReplayedDataTime:.z.P+.global.wipeOfReplayedDataGap;
		.global.totalRowCount:count[dxOrderPublic_file];
...
/trigger timer every 200ms

masterHandle:hopen`$"::",string[55555+.proc.portNumberIncrement];
